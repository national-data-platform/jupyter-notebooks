...
class IncrementalModel:
    ...
    def update_data(self, new_data):
        ...
        # Ensure there's at least one data point already stored
        if not self.all_data.empty:
            # Get the most recent timestamp in the existing data
            latest_timestamp = self.all_data['timestamp'].max()
            
            # Filter out any new data points that are older than the latest timestamp
            new_df = new_df[new_df['timestamp'] > latest_timestamp]
        
        # If after filtering there are no new data points to add, simply return
        if new_df.empty:
            return
    
        # Concatenate new data points that passed the filter
